...
import { SONG_URL, ERROR_PAGE } from '../../../apiUrls';
    ...
    <>
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          name: Yup.string().matches(/^[a-zA-Z0-9_\s]+$/, 'Invalid name format').max(100).required('Name is required'),
          artist: Yup.string()
          ...
          try {
            if (scriptedRef.current) {
              ...
              if (nullable_playlistId !== undefined) {
                formData.append('playlist', nullable_playlistId);
              }
              if (selectedImage) {
              ...
              const url = id === 'new' ? SONG_URL : `${SONG_URL}/${id}`;
